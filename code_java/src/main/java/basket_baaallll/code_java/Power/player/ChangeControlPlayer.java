...
public class ChangeControlPlayer extends PowerPlayer {

    private int timer = 5;
    private final HashMap<String, KeyCode> initControl = new HashMap<>();


    public ChangeControlPlayer(Player player) {
        super(player);
    ...
    @Override
    public void use() {
        player.setControl(initControl.get("right"),initControl.get("left"),initControl.get("jump"),initControl.get("power"));
